...
while True:
    ...
    for (x, y, w, h) in faces_rect:
        ...
        logging.debug(f"Emocao detectada: {emotion} com score {score}")

        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, f"{name} ({confidence_text})", (x, y - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

        if score is not None:
            cv2.putText(frame, f"Emoção: {emotion} ({round(score*100)}%)", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        else:
            cv2.putText(frame, "Emoção: Indefinida", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
