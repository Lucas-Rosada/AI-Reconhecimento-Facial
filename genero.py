...
while True:
    ...
    for (x, y, w, h) in faces_rect:
        ...
        try:
            ...
            logging.error(f"Erro ao detectar gênero: {e}")
            gender = "Desconhecido"

        logging.debug(f"Gênero detectado: {gender}")

        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, f"{name} ({confidence_text}) - {gender}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
