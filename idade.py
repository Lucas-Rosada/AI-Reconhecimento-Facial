...
while True:
    ...
    for (x, y, w, h) in faces_rect:
        ...
        id, confidence = face_recognizer.predict(gray_face_roi)
        ...
        try:
            age_predictions = DeepFace.analyze(face_roi, actions=['age'])
            age = age_predictions[0]['age']
        except Exception as e:
            logging.error(f"Erro ao estimar idade: {e}")
            age = "Desconhecida"

        logging.debug(f"Reconhecido: {name} com confiança {confidence_text}, Idade: {age}")

        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, f"{name} ({confidence_text}), Idade: {age}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
