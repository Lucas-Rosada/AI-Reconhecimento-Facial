...
while True:
    ...
    for (x, y, w, h) in faces_rect:
        ...
        if confidence < 100:
            ...
            try:
                analysis = DeepFace.analyze(img_path=face_img, actions=['age', 'gender', 'emotion', 'race'], detector_backend='opencv')[0]
                ...
                cv2.putText(frame, f"Age: {age}", (x, y - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                cv2.putText(frame, f"Gender: {gender}", (x, y - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                cv2.putText(frame, f"Emotion: {emotion}", (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                cv2.putText(frame, f"Race: {race}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            except Exception as e:
                logging.error(f"Erro ao analisar a face com DeepFace: {e}")
        else:
            name = "Desconhecido"
            confidence_text = f"{round(100 - confidence)}%"
        ...
        cv2.putText(frame, f"{name} ({confidence_text})", (x, y - 65), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
